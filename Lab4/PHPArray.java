...
public class PHPArray<V> implements Iterable<V> {
  ...
  public PHPArray(final int capacity) {
    M = capacity;
  ...
  private void resize(final int capacity) {
    final PHPArray<V> temp = new PHPArray<V>(capacity);
  ...
  private class Node<V> {
    ...
    Node(final String key, final V value){
      this(key, value, null);
    }

    Node(final String key, final V value, final Node<V> next){
      this.key = key;
      this.value = value;
      this.next = next;
    }
  ...
  public class MyIterator implements Iterator<V> {
    ...
    public V next() {
      final V result = current.value;
      current = current.next;
      return result;
    }
  }

  private static <V> void show(final PHPArray<V> array){
  ...
  public static void main(final String[] args) {
      final PHPArray<Integer> A = new PHPArray<>(2);
