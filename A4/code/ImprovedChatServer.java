 ...
 * Assignment 4 is similar to this, but with handshaking and encryption (and a
 * couple other minor improvements) as specified in the Assignment sheet. This
 * server (together with ImprovedChatClient) will run as is. Run them to see how
 * the messages appear and how clients connect and disconnect. Adapted from Dr.
 * John Ramirez's CS 1501 Assignment 4
 */
public class ImprovedChatServer {
				...
				if (numUsers < MaxUsers) {
					try {
						...
						synchronized (this) {
							...
							threads[numUsers] = new UserThread(newSocket, numUsers, newName, tempReader);
	...
	private class UserThread extends Thread {
		...
		private UserThread(Socket newSocket, int id, String newName, BufferedReader newReader) throws IOException {
			mySocket = newSocket;
			myId = id;
			myName = newName;
			myReader = newReader;
			myWriter = new PrintWriter(new BufferedWriter(new OutputStreamWriter(mySocket.getOutputStream())), true);
		}
